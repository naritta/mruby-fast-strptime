...
**
** Copyright (c) Tadayoshi Funaba, Koichi Sasada, Yui Naruse, Ritta Narita 2011-2017
**
...
static int valid_range_p(int v, int a, int b) {
    return !(v < a || v > b);
}
...
void
mrb_timespec_now(mrb_state *mrb, struct timespec *ts)
{
...
static struct mrb_data_type mrb_strptime_type = {
  "Strptime", mrb_free
};

static int
strptime_exec0(mrb_state *mrb, void **pc, const char *fmt, const char *str, size_t slen,
...
void **
strptime_compile(mrb_state *mrb, const char *fmt, size_t flen)
{
    size_t fi = 0;
    char c;
	...
	case '%':
	    ...
	    default: mrb_raise(mrb, E_RUNTIME_ERROR, "invalid format"); break;
	    }
	case ' ':
	case '\t':
	case '\n':
...
static mrb_value strptime_exec(mrb_state *mrb, mrb_value self) {
  ...
  const char *str = mrb_str_to_cstr(mrb, mstr);
    ...
    r = strptime_exec0(mrb, tobj->isns, tobj->fmt, str,
                   strlen(str), &ts, &gmtoff);

    struct RClass *time_class;
    time_class = mrb_class_get(mrb, "Time");
    mrb_value time_obj = mrb_obj_value(time_class);

    mrb_value time = mrb_funcall(mrb, time_obj, "at", 1, mrb_fixnum_value(ts.tv_sec));
    return time;
}
